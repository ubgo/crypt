...
func TestDecrypt_TamperedCiphertext_BadPadding(t *testing.T) {
	enc, _ := EncryptWithKey(key256, "secret")
	// Flip the last byte. Decryption succeeds at AES level but PKCS#7
	// padding becomes invalid, surfacing as ErrInvalidPadding.
	tampered := enc[:len(enc)-2] + "ff"
	if _, err := DecryptWithKey(key256, tampered); err == nil {
